(function(){
	...
	'use strict';
	angular.module('i94').controller('HistoryController', HistoryController);

	HistoryController.$inject = ['historySearchService', '$uibModal', '$scope','$state', '$translate'];

	function HistoryController(historySearchService, $uibModal, $scope,  $state, $translate) {
		var vm = this;
		vm.controllerName = "HistoryController";
...
//				alert("call searchHistory");
				historySearchService.searchHistory(vm.document).then(success, failed);

				function success(data){
					vm.setIsSubmitButtonDisabled(false);
					if(data==true){
//						alert("call requestResults");
