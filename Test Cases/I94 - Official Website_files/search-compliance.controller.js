(function(){
	...
	'use strict';
	angular.module('i94').controller('SearchComplianceController', SearchComplianceController);

	SearchComplianceController.$inject = ['historySearchService', '$uibModal', '$scope','$state', '$translate'];
	
	//PINF-622
	function SearchComplianceController(historySearchService, $uibModal, $scope,  $state, $translate) {
		var vm = this;
		vm.controllerName = "SearchComplianceController";
		...
		function onSubmit(){
			...
			if (vm.form.$valid && vm.validation && vm.minimumYearValid ) {
				...
				vm.document.hasError=false;
				
				vm.setIsSubmitButtonDisabled(true);
				historySearchService.searchOverstayCompliance(vm.document).then(success, failed);
				
				function success(data){
					vm.setIsSubmitButtonDisabled(false);
					if(data==true){
						//historySearchService.requestResults();
						if (vm.document.hasError == true)
						{
							$state.go('compliance');
						}
						else
						   $state.go('compliance-results');
					} else {
